...
智能路灯后端服务（Spring Boot + MySQL + MQTT）
...
```bash
docker run -d --name smartlamp-mysql \
  -p 3307:3306 \
...
```bash
docker exec -it smartlamp-mysql mysql -uroot -p123456 \
  -e "CREATE DATABASE IF NOT EXISTS smartlamp DEFAULT CHARACTER SET utf8mb4;"
```

> 后端 JPA 会自动建表，但不会自动创建数据库，此步骤必须执行。
...
1. 使用 IDEA 打开项目根目录（包含 `pom.xml`）。
2. 等待 Maven 依赖下载完成。
3. 找到启动类 `SmartlampApplication`，右键运行。
4. 等待控制台输出 `Tomcat started on port 8080`。
...
数据库 URL 中已添加 `allowPublicKeyRetrieval=true`，若仍报错，检查 MySQL 容器是否启动，端口是否映射为 3307。
